...
router.post("/", async (req, res) => {
  if (req.currentUser?.role !== "admin") {
    return res.status(403).json({ message: "You cant add a palce" });
  }
...
router.put("/:placeId", async (req, res) => {
  if (req.currentUser?.role !== "admin") {
    return res.status(403).json({ message: "You cant edit a palce" });
  }
...
router.delete("/:placeId", async (req, res) => {
  if (req.currentUser?.role !== "admin") {
    return res.status(403).json({ message: "You cant delete a palce" });
  }
